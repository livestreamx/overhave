...
class TestSynchronizer:
    def test_update_feature(
        ...
        mock_synchronizer: OverhaveSynchronizer,
    ):
        ...
        with patch("overhave.synchronization.synchronizer.get_current_time", return_value=feature_file_ts):
            with patch("overhave.synchronization.synchronizer.db.create_session", return_value=MagicMock()):
                mock_synchronizer._update_feature(mock_feature_model, mock_info, feature_file_ts)
    ...
    def test_synchronize(self, synchronizer_storage_manager: MagicMock, mock_synchronizer: OverhaveSynchronizer):
        with patch.object(mock_synchronizer, "_git_initializer"):
            with patch.object(mock_synchronizer, "_extract_recursively", return_value=[]):
                with patch.object(mock_synchronizer, "_storage_manager") as mock_storage_manager:
                    ...
                    with patch.object(mock_synchronizer, "_update_feature") as mock_update_feature:
                        mock_synchronizer.synchronize(create_db_features=True, pull_repository=True)
