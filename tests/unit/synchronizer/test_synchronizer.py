...
class TestSynchronizer:
    def test_update_feature(
        ...
        with (patch("overhave.synchronization.synchronizer.get_current_time", return_value=feature_file_ts),
              patch("overhave.synchronization.synchronizer.db.create_session", return_value=MagicMock())):
    ...
    def test_synchronize(self, synchronizer_storage_manager: MagicMock, overhave_synchronizer: OverhaveSynchronizer):
        with (patch.object(overhave_synchronizer, "_git_initializer"),
              patch.object(overhave_synchronizer, "_extract_recursively", return_value=[]),
              patch.object(overhave_synchronizer, "_storage_manager") as mock_storage_manager,
              patch.object(overhave_synchronizer, "_update_feature") as mock_update_feature):
