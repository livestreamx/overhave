...
from overhave.publication.gitlab.tokenizer.client import TokenizerRequestParamsModel, TokenizerClient, InvalidUrlException, InvalidRemoteKeyNameException
...
class TestTokenizerClient:
    ...
    @pytest.mark.parametrize(
        ("enabled", "url", "initiator", "remote_key", "remote_key_name"),
        [(False, None, "Overhave", "a", None)]
    )
    ...
    @pytest.mark.parametrize(
        ("enabled", "url", "initiator", "remote_key", "remote_key_name"),
        [(False, "edu.ru", "Overhave", "abaca", None)]
    )
