...
class TestEmulationStorage:
    ...
    def test_create_emulation_run(
        ...
        with create_test_session() as session:
            ...
            assert emulation_run.status == db.EmulationStatus.CREATED
    ...
    def test_get_requested_emulation_run(
        ...
        assert requested_emulation_run.status == db.EmulationStatus.REQUESTED
    ...
    def test_set_error_run_status(
        ...
        assert test_emulation_run.status is db.EmulationStatus.CREATED
        with count_queries(1):
        ...
        with create_test_session() as session:
            ...
            assert emulation_run.status == db.EmulationStatus.ERROR

    @pytest.mark.parametrize(
        "emulation_status",
        [db.EmulationStatus.READY, db.EmulationStatus.REQUESTED, db.EmulationStatus.ERROR, db.EmulationStatus.CREATED],
    )
    def test_set_emulation_run_status(
        self,
        test_emulation_storage: EmulationStorage,
        test_emulation_run: EmulationRunModel,
        emulation_status: db.EmulationStatus,
    ) -> None:
        with count_queries(1):
